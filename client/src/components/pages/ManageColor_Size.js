...
function ManageColor_Size() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const endpoint = `${API_URL}/api/${selectedOption}s`;
    ...
    const fetchData = async (id, url, errorMessage) => {
        try {
            ...
            if (response.status === 200) {
                setVisible(true);
                setTimeout(() => {
                    nameRef.current.value = selectedOption === 'color' ? response.data.color.color_name : response.data.size.size_name;
                });
    ...
    const handleClickEdit = async (id) => {
        const url = selectedOption === 'color' ? `${API_URL}/api/colors/${id}` : `${API_URL}/api/sizes/${id}`;
    ...
    const handleConfirmDelete = async (id) => {
        try {
            ...
            if (selectedOption === 'color') {
                endpoint = `${API_URL}/api/colors/${id}`;
                successMessage = 'màu sắc';
            } else {
                endpoint = `${API_URL}/api/sizes/${id}`;
            ...
            if (response.status === 200) {
                toast(`Xóa dữ liệu ${successMessage} thành công`);
                setIsReload(!isReload);
            } else {
                toast(`Lỗi khi xóa ${successMessage}`);
            }
        } catch (e) {
            toast(e.response.data.message);
        }
    ...
    const clickBtnAdd_Edit = () => {
        ...
        if (selectedOption === 'color') {
            endpoint = `${API_URL}/api/colors`;
        } else {
            endpoint = `${API_URL}/api/sizes`;
        }
        ...
        if (idItem === 0) {
            axios.post(endpoint, updatedInfo)
                .then((response) => {
                    if (response.status === 201) {
                        ...
                        toast('Lỗi khi thêm mới dữ liệu');
                    }
                })
                .catch((e) => {
                    toast(e.response.data.message);
            ...
            axios.put(`${endpoint}/${idItem}`, updatedInfo).then((response) => {
                if (response.status === 200) {
                    ...
                    toast('Lỗi khi cập nhật dữ liệu');
                }
            })
                .catch((e) => {
                    toast(e.response.data.message);
